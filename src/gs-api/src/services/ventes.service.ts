...
import { LigneVenteDto } from '../models';
@Injectable({
  providedIn: 'root',
})
class VentesService extends __BaseService {
  ...
  findAllResponseLigne(id: number): __Observable<__StrictHttpResponse<Array<LigneVenteDto>>> {
    let __params = this.newParams();
    let __headers = new HttpHeaders();
    let __body: any = null;
    let req = new HttpRequest<any>(
      'GET',
      this.rootUrl + `/gestiondestock/v1/ventes/AllligneVente/${id}`,
      __body,
      {
        headers: __headers,
        params: __params,
  ...
  findlignebyIdvente(id: number): __Observable<Array<LigneVenteDto>> {
    return this.findAllResponseLigne(id).pipe(
